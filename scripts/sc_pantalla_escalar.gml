...
/* Dependiendo si tiene bordes negros a los costados, se dibuja el color solido de fondo (caso contrario no ya que consumiria recursos de más)
NOTA: Esto es propio de este juego, no confundir con el paquete de adaptación de pantalla
*/
if( view_wview[0] > room_width or view_hview[0] > room_height ){ sc_view_adaptar_room(true); }
else{ sc_view_adaptar_room(false) }

//Por último se usa variables globales relativas a lo que se ve en la view (que a veces es mas ancho/alto que el de la room)
//Nota: es importante esto para posicionar objetos o usar el evento normal draw.
g_view_x = view_xview[0] + g_gui_x;
g_view_y = view_yview[0] + g_gui_y;

g_view_ancho = view_wview[0] - g_gui_x * 2;
g_view_alto = view_hview[0] - g_gui_y * 2;

surface_resize(application_surface, actual_ancho, actual_alto)
